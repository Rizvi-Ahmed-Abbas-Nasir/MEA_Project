  ...
  return (
    <>
      {session?.user?.role === "admin" ? (
        <div className="flex xl:flex-row flex-col w-full h-screen  text-xl bg-gray-100">
          <NAV />
                  ...
                  <tbody className="text-gray-600 text-sm font-light">
                    {members.map((member, index) => (
                      <tr key={member.id} className={`hover:bg-gray-100 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                        <td className="py-3 px-4 border-b">{member.employee_id}</td>
                        <td className="py-3 px-4 border-b">{member.fullName}</td>
                        <td className="py-3 px-4 border-b">{member.email}</td>
                        <td className="py-3 px-4 border-b">{member.contactNumber}</td>
                        <td className="py-3 px-4 border-b">{member.designation}</td>
                        ...
                        <td className="py-3 px-4 border-b">
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded flex items-center gap-2 transition duration-300 ease-in-out"
                            >
                              <i className="fas fa-check"></i> Accept
                            </button>
                            <button
                              onClick={() => handleDecline(member.id)}
                              className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded flex items-center gap-2 transition duration-300 ease-in-out"
                            >
